...
//     // ✅ FIX: preserve LEFT JOIN when no appointments exist
//     if (role !== UserRole.SUPER_ADMIN) {
//       qb.andWhere(
//         '(lead.tenantId = :tenantId OR lead.id IS NULL)',
//         { tenantId },
//       );
//     }
...
//     if (startDate) {
//       qb.andWhere('appointment.date >= :start', { start: startDate });
//     }

//     if (endDate) {
//       qb.andWhere('appointment.date <= :end', { end: endDate });
//     }
...
//     // ✅ Only ACTIVE STAFF (not organisation/admin)
//     const staffList = await this.userRepository.find({
//       where: { tenantId, status: 'active' as any },
//     });
...
//     const bookedStaffIds = slotAppointments.map((a) => a.staffId);
//     const freeStaff = eligibleStaff.filter(
//       (s) => !bookedStaffIds.includes(s.id),
//     );
...
//     const assignedStaffId = freeStaff[0].id;
...
//     if (
//       role !== UserRole.SUPER_ADMIN &&
//       appointment.lead?.tenantId !== tenantId
//     ) {
//       throw new NotFoundException('Appointment not found');
//     }
